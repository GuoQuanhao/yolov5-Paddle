...
Usage:
    ...
    $ python segment/val.py --weights yolov5s-seg.pdparams --data coco.yaml --img 640  # validate COCO-segments

Usage - formats:
    $ python segment/val.py --weights yolov5s-seg.pdparams           # PaddlePaddle
                                      yolov5s-seg.onnx               # ONNX Runtime or OpenCV DNN with --dnn
                                      yolov5s-seg_openvino_label     # OpenVINO
                                      yolov5s-seg.engine             # TensorRT
                                      yolov5s-seg_paddle_model       # PaddleInference
                                      yolov5s-seg.nb                 # PaddleLite
"""
...
from utils.paddle_utils import de_parallel, select_device, smart_inference_mode
...
def process_batch(detections, labels, iouv, pred_masks=None, gt_masks=None, overlap=False, masks=False):
    ...
    if masks:
        if overlap:
            ...
            gt_masks = gt_masks.tile([nl, 1, 1])  # shape(1,640,640) -> (n,640,640)
            gt_masks = paddle.where(gt_masks == index, 1.0, 0.0).astype("float32")
        if gt_masks.shape[1:] != pred_masks.shape[1:]:
            gt_masks = F.interpolate(gt_masks[None], pred_masks.shape[1:], mode='bilinear', align_corners=False)[0]
            gt_masks = (gt_masks > 0.5).astype("float32")
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, (pred, proto) in enumerate(zip(preds, protos)):
            ...
            correct_masks = paddle.zeros([npr, niou.item()], dtype=paddle.bool)  # init
            correct_bboxes = paddle.zeros([npr, niou.item()], dtype=paddle.bool)  # init
            seen += 1

            if npr == 0:
                if nl:
                    stats.append((correct_masks, correct_bboxes, *paddle.zeros((2, 0)), labels[:, 0]))
                    if plots:
            ...
            pred_masks = pred_masks.astype(paddle.uint8).numpy()
            ...
            if save_json:
                pred_masks = scale_image(im[si].shape[1:],
                                         pred_masks.transpose((1, 2, 0)), shape, shapes[si][1])
        ...
        if plots and batch_i < 3:
            if len(plot_masks):
                plot_masks = np.concatenate(plot_masks, axis=0)
    ...
    # Compute metrics
    stats = [paddle.concat(x, 0).cpu().numpy() for x in zip(*stats)]  # to numpy
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s-seg.pdparams', help='model path(s)')
