...
from utils.paddle_utils import select_device, time_sync
...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if len(pred) == 0:
                if nl:
                    stats.append((np.zeros([0, niou], dtype=np.bool_), np.array([], dtype=np.float32), np.array([], dtype=np.float32), tcls))
                continue
    ...
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats, plot=False, save_dir=save_dir, names=names) # AIStudio set plot=False
...
def parse_opt():
    ...
    opt.data, opt.hyp, opt.cfg = check_yaml(opt.data), check_yaml(opt.hyp), check_yaml(opt.cfg)  # check YAML
