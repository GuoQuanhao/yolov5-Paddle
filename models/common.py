...
from utils.paddle_utils import time_sync
...
class Conv(nn.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.act = nn.Silu() if act is True else (act if isinstance(act, nn.Layer) else Identity())
...
class TransformerBlock(nn.Layer):
    ...
    def forward(self, x):
        if self.conv is not None:
            x = self.conv(x)
        b, _, w, h = x.shape
        p = x.flatten(2).unsqueeze(0).transpose((0, 3)).squeeze(3) # 注意
        return self.tr(p + self.linear(p)).unsqueeze(3).transpose((0, 3)).reshape((b, self.c2, w, h)) # 注意
...
class GhostBottleneck(nn.Layer):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False),
                                      Conv(c1, c2, 1, 1, act=False)) if s == 2 else Identity()
...
class Contract(nn.Layer):
    ...
    def forward(self, x):
        ...
        s = self.gain
        x = x.reshape([b, c, h // s, s, w // s, s])  # x(1,64,40,2,40,2)
        x = x.transpose([0, 3, 5, 1, 2, 4])  # x(1,2,2,64,40,40)
        return x.reshape(b, c * s * s, h // s, w // s)  # x(1,256,40,40)
...
class Expand(nn.Layer):
    ...
    def forward(self, x):
        ...
        s = self.gain
        x = x.reshape([b, s, s, c // s ** 2, h, w])  # x(1,2,2,16,80,80)
        x = x.transpose([0, 3, 4, 1, 5, 2])  # x(1,16,80,2,80,2)
        return x.reshape([b, c // s ** 2, h * s, w * s])  # x(1,16,160,160)
...
class AutoShape(nn.Layer):
    # YOLOv5 input-robust model wrapper for passing cv2/np/PIL/Paddle inputs. Includes preprocessing, inference and NMS
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        #   paddle:           = paddle.zeros([16,3,320,640])  # BCHW (scaled to size=640, 0-1 values)
        #   multiple:        = [Image.open('image1.jpg'), Image.open('image2.jpg'), ...]  # list of images

        t = [time_sync()]
        p = self.model.parameters()[0].place  # for device and type
        if isinstance(imgs, paddle.Tensor):  # paddle
            with paddle.amp.auto_cast(enabled='CUDAPlace' in str(p)):
        ...
        for i, im in enumerate(imgs):
            ...
            if im.shape[0] < 5:  # image in CHW
                im = im.transpose((1, 2, 0))  # reverse dataloader .transpose(2, 0, 1) # 注意
        ...
        x = np.ascontiguousarray(x.transpose((0, 3, 1, 2)))  # BHWC to BCHW # 注意
        x = paddle.to_tensor(x).to(p.device).type_as(p) / 255.  # uint8 to fp16/32
        t.append(time_sync())
...
class Detections:
    ...
    def __init__(self, imgs, pred, files, times=None, names=None, shape=None):
        ...
        gn = [paddle.to_tensor([*(im.shape[i] for i in [1, 0, 1, 0]), 1., 1.], device=d) for im in imgs]  # normalizations
