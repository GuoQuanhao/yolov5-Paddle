...
from utils.download_utils import download_url_to_file
...
# matplotlib.use('Agg')  # for writing to files only, '#' this line if you use AIStudio
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    ...
    if isinstance(images, paddle.Tensor):
        images = images.cpu().astype('float32').numpy()
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            blocks = paddle.chunk(x[0].cpu(), channels, axis=0)  # select batch index 0, block by channels
