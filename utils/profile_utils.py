...
def counter_conv(bias, kernel_size, output_size, in_channel, group):
    ...
    return int(output_size * (in_channel / group * kernel_size + bias))


def count_parameters(m, x, y):
    total_params = 0
    for p in m.parameters():
        total_params += p.numel().item()
    m.total_params[0] = counter_parameters(m.parameters())


def count_adap_avgpool(m, x, y):
    kernel = paddle.to_tensor(
        [*(x[0].shape[2:])], dtype=paddle.int32) // paddle.to_tensor([*(y.shape[2:])], dtype=paddle.int32)
...
def paddle_profile(model, inputs, custom_ops=None, verbose=True, ret_layer_info=False, report_missing=False):
    handler_collection = {}
    ...
    def add_hooks(m):
        m.register_buffer('total_ops', paddle.zeros([1], dtype=paddle.get_default_dtype()))
        m.register_buffer('total_params', paddle.zeros([1], dtype=paddle.get_default_dtype()))
