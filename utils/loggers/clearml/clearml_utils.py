...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        if self.clearml:
            self.task = Task.init(
                ...
                auto_connect_frameworks={'paddle': False}
                # We disconnect paddle auto-detection, because we added manual model save points in the code
            )
    ...
    def log_image_with_boxes(self, image_path, boxes, class_names, image, conf_threshold=0.25):
        ...
        image (Tensor): A paddle tensor containing the actual image data
        """
        if len(self.current_epoch_logged_images) < self.max_imgs_to_log_per_epoch and self.current_epoch >= 0:
            ...
            if self.current_epoch % self.bbox_interval == 0 and image_path not in self.current_epoch_logged_images:
                im = np.ascontiguousarray(np.moveaxis((image * 255).clip(0, 255).astype("uint8").cpu().numpy(), 0, 2))
